...
export default function HowWorks() {
  return (
    <section
      id="how"
      className="py-16 md:py-24 bg-gradient-to-b from-white via-[#F7FBFF] to-white relative overflow-hidden"
    >
      {/* Decorative Blur */}
      <div className="absolute -top-32 left-1/2 -translate-x-1/2 w-[500px] h-[500px] bg-red-200 opacity-10 blur-[120px]" />
...
<div className="max-w-4xl mx-auto px-6 relative">
  ...
  <div
    className="
      absolute
      left-8 md:left-1/2
      md:-translate-x-1/2       /* only center on md+ */
      top-0 bottom-0
      w-0.5 bg-red-200
      z-0
    "
  ...
  {/* Optional: fade the line out so it doesn't go below step 4 */}
  <div
    className="
      pointer-events-none
      absolute
      left-8 md:left-1/2
      md:-translate-x-1/2        /* same center as line */
      bottom-0
      w-8 h-20
      bg-gradient-to-t from-white via-[#F7FBFF] to-transparent
      z-10
    "
  />
    ...
    >
      ...
      <div
        className="
          absolute
          left-8 md:left-1/2
          md:-translate-x-1/2      /* center only on md+ */
          z-20
      ...
      {/* Content Card */}
      <div
        className={`w-full ml-16 md:ml-0 p-4 md:p-6 bg-white rounded-2xl shadow-md border border-red-100/50 hover:border-red-300 transition-all text-left text-sm sm:text-base md:text-[16px] lg:text-[16px] ${
          index % 2 === 0
            ? "md:w-[calc(50%-4rem)] md:pr-10"
            : "md:w-[calc(50%-4rem)] md:pl-10 order-2"
        }`}
