...
export const config= {
    name:"PaidJob-fetch-trending-videos",
    type:'event',
    ...
    infrastructure: {
        handler: { 
            timeout: 120  // yt not respond give up after 30s
        },
        queue: { 
            maxRetries: 3,  // try 3 times if failed
            visibilityTimeout: 130  // wait 130s then again try
        }
    }
}
    ...
    //now setting the everything for the job : jobId the data is {---}
        ...
        if(!YtTrendingVideos.items || YtTrendingVideos.items.length === 0){
            ...
            await state.set('PaidJobs', PaidJobId,{
                ...
                TrendingVideos: TrendingVideos.slice(0, 8),   //setting trending videos for next event to use in prompt
                region              //for checking.
            })
